-- ============================================================================================
-- PROPOSED — _delete_project must VALIDATE the reassign target  ·  ROLLED-BACK PROOF
-- 39 assertions. NOTHING IS APPLIED. The whole file is one transaction ending in ROLLBACK.
-- Proves PROPOSED_delete_project_validate_reassign_target.sql against the CURRENTLY LIVE
-- 20260716104604 body.
-- ============================================================================================
-- SHAPE (house convention):
--   (1) fixtures + harness + ANTI-VACUITY, then a RED phase run against the LIVE function
--       (verified live 2026-07-19: private._delete_project contains no 'does not exist' check and
--        public.delete_project's p_reassign_to default is still 'other'::text)
--   (2) the DDL UNDER TEST at TOP LEVEL — a bare CREATE OR REPLACE FUNCTION cannot run inside a
--       plpgsql DO block, and it must land AFTER the RED phase so RED really tests today's rules
--   (3) a GREEN phase re-running the same scenarios plus the full regression surface
--   (4) a VERDICT that RAISES on any NULL pass or an unexpected assertion count
--
...
-- ---------------------------------------------------------------------------
-- (1) FIXTURES + HARNESS GUARD + ANTI-VACUITY + THE RED PHASE (LIVE FUNCTION)
-- ---------------------------------------------------------------------------
do $red$
declare
...
begin
  ...
  -- Confirm we really are testing the pre-fix body, so RED cannot pass for the wrong reason.
  if (select position('reassign target project does not exist' in pg_get_functiondef(p.oid))
      from pg_proc p join pg_namespace n on n.oid = p.pronamespace
      where n.nspname = 'private' and p.proname = '_delete_project') > 0 then
    raise exception 'RED PHASE INVALID: the live function already contains the fix'; end if;

  -- ============================================================================
  -- RED — the disease, against the CURRENTLY LIVE rules
  -- ============================================================================
  ...
  insert into _r values (1,'R01 RED: live fn ACCEPTS a nonexistent reassign target','ALLOWED',v_actual,v_actual='ALLOWED');
  insert into _r values (2,'R02 RED: and reports success (tasks_affected=2, project_deleted=true)','2|true',
    coalesce(v_i1::text,'NULL')||'|'||coalesce(v_b1::text,'NULL'), v_i1=2 and v_b1 is true);
  insert into _r values (3,'R03 RED: SILENT STRANDING — both moved tasks now point at an unresolvable project','2',
    coalesce(v_i2::text,'NULL'), v_i2=2);
  insert into _r values (4,'R04 RED: the project row was deleted anyway (damage is committed, not partial)','0',
  ...
  insert into _r values (5,'R05 RED: live fn ACCEPTS a target belonging to ANOTHER workspace','ALLOWED|'||v_bonly,
    v_actual||'|'||coalesce(v_s1,'NULL'), v_actual='ALLOWED' and v_s1=v_bonly);

  -- R06: the phantom seed default. p_reassign_to OMITTED -> today's default 'other', which (checked
  -- live 2026-07-19) exists in NONE of the three real workspaces either.
  begin
    perform pg_temp.imp(v_owner);
  ...
  insert into _r values (6,'R06 RED: omitted p_reassign_to strands tasks on the phantom seed id other','ALLOWED|other',
  ...
  insert into _r values (7,'R07 RED anti-vacuity: a VALID target already worked pre-fix','ALLOWED|'||v_other,
...
-- ---------------------------------------------------------------------------
-- (2) THE DDL UNDER TEST — top level (a bare CREATE OR REPLACE cannot run in a DO block).
--     Byte-identical to PROPOSED_delete_project_validate_reassign_target.sql. Rolled back with
--     everything else at the end of this file.
-- ---------------------------------------------------------------------------
...
-- ---------------------------------------------------------------------------
-- (3) GREEN — the cure, plus the full regression surface
-- ---------------------------------------------------------------------------
do $green$
declare
...
begin
  ...
  insert into _r values (8,'G01 nonexistent target now rejected — EXACT sqlstate AND message','P0002|'||c_nofix,
    v_actual||'|'||v_msg, v_actual='P0002' and v_msg=c_nofix);
  insert into _r values (9,'G02 rejected call is atomic: all 3 WS_T slug tasks still on the original project','3',
    v_i1::text, v_i1=3);
  insert into _r values (10,'G03 rejected call is atomic: the project row was NOT deleted (no half-done delete)','1',
    v_i2::text, v_i2=1);
  ...
  insert into _r values (11,'G04 cross-workspace target rejected — EXACT sqlstate AND message','P0002|'||c_nofix,
    v_actual||'|'||v_msg, v_actual='P0002' and v_msg=c_nofix);
  insert into _r values (12,'G05 cross-workspace rejection atomic: WS_T tasks+project and the WS_B tenant untouched',
  ...
  exception when others then v_actual := sqlstate; v_msg := sqlerrm; end;
  execute 'reset role';
  insert into _r values (13,'G06 omitted p_reassign_to -> 22023 required (NOT P0002; default is NULL now)',
  ...
  insert into _r values (14,'H01 valid target: mode unchanged','unassign',coalesce(v_s1,'NULL'),v_s1='unassign');
  insert into _r values (15,'H02 valid target: tasks_affected=2','2',coalesce(v_i1::text,'NULL'),v_i1=2);
  insert into _r values (16,'H03 valid target: project_deleted=true','true',coalesce(v_b1::text,'NULL'),v_b1 is true);
  insert into _r values (17,'H04 result jsonb key set UNCHANGED','mode,project_deleted,tasks_affected',
    coalesce(v_s2,'NULL'), v_s2='mode,project_deleted,tasks_affected');
  insert into _r values (18,'H05 the 2 visible tasks are re-filed onto the valid target','2',
    coalesce(v_i2::text,'NULL'), v_i2=2);
  insert into _r values (19,'H06 GATE A preserved: hidden private task NOT re-filed',v_slug,
    coalesce(v_s3,'NULL'), v_s3=v_slug);
  insert into _r values (20,'H07 WORKSPACE SCOPE preserved: WS_B same-slug victim untouched',v_slug,
    coalesce(v_actual,'NULL'), v_actual=v_slug);
  insert into _r values (21,'H08 the project row is deleted','0',coalesce(v_i5::text,'NULL'),v_i5=0);
  ...
  insert into _r values (22,'H09 ADMIN (rank 2) can still unassign with a valid target','2|0',
  ...
  insert into _r values (23,'C01 cascade with a GARBAGE reassign target still succeeds (target ignored)','2|true',
    coalesce(v_i1::text,'NULL')||'|'||coalesce(v_b1::text,'NULL'), v_i1=2 and v_b1 is true);
  insert into _r values (24,'C02 cascade GATE A + tenant scope intact: hidden task and WS_B victim survive','1|1',
  ...
  insert into _r values (25,'C03 cascade with a NULL reassign target still succeeds','2|true',
    coalesce(v_i1::text,'NULL')||'|'||coalesce(v_b1::text,'NULL'), v_i1=2 and v_b1 is true);
  ...
  exception when others then v_actual := sqlstate; v_msg := sqlerrm; end;
  execute 'reset role';
  insert into _r values (26,'C04 admin cascade still denied (owner-only gate untouched)','42501|'||c_casrank,
  ...
  exception when others then v_actual := sqlstate; v_msg := sqlerrm; end;
  execute 'reset role';
  insert into _r values (27,'E01 explicit NULL target -> 22023 required (not the new P0002)','22023|'||c_required,
  ...
  exception when others then v_actual := sqlstate; v_msg := sqlerrm; end;
  execute 'reset role';
  insert into _r values (28,'E02 blank target -> 22023 required (not the new P0002)','22023|'||c_required,
    v_actual||'|'||v_msg, v_actual='22023' and v_msg=c_required);
  ...
  exception when others then v_actual := sqlstate; v_msg := sqlerrm; end;
  execute 'reset role';
  insert into _r values (29,'E03 target == deleted project -> 22023 differ','22023|'||c_differ,
  ...
  exception when others then v_actual := sqlstate; v_msg := sqlerrm; end;
  execute 'reset role';
  insert into _r values (30,'E04 deleted project missing -> P0002 project not found','P0002|'||c_notfnd,
  ...
  exception when others then v_actual := sqlstate; v_msg := sqlerrm; end;
  execute 'reset role';
  insert into _r values (31,'E05 PRECEDENCE unchanged: both ids bogus -> project not found wins','P0002|'||c_notfnd,
    v_actual||'|'||v_msg, v_actual='P0002' and v_msg=c_notfnd);
  ...
  exception when others then v_actual := sqlstate; v_msg := sqlerrm; end;
  execute 'reset role';
  insert into _r values (32,'E06 non-member -> 42501 not authorized','42501|'||c_member,
  ...
  exception when others then v_actual := sqlstate; v_msg := sqlerrm; end;
  execute 'reset role';
  insert into _r values (33,'E07 member (rank 1) -> 42501 rank gate','42501|'||c_unarank,
  ...
  exception when others then v_actual := sqlstate; v_msg := sqlerrm; end;
  execute 'reset role';
  insert into _r values (34,'E08 guest (rank 0) -> 42501 rank gate','42501|'||c_unarank,
    v_actual||'|'||v_msg, v_actual='42501' and v_msg=c_unarank);
  ...
  exception when others then v_actual := sqlstate; v_msg := sqlerrm; end;
  execute 'reset role';
  insert into _r values (35,'E09 invalid mode -> 22023','22023|invalid mode: bogus',
  ...
  insert into _r values (36,'X01 anti-vacuity: the denied outsider CAN delete in their OWN workspace','true',
  ...
  insert into _r values (37,'P01 private._delete_project: DEFINER + search_path='''' + authenticated-only',
  ...
  insert into _r values (38,'P02 public.delete_project: INVOKER + search_path='''' + authenticated-only',
  ...
  select pg_get_function_arguments(p.oid) into v_actual
    from pg_proc p join pg_namespace n on n.oid = p.pronamespace
    where n.nspname='public' and p.proname='delete_project';
  insert into _r values (39,'P03 signature default is NULL — the phantom seed id is gone from the API',
...
begin
  ...
  select count(*) into v_null from _r where pass is null;
  if v_null > 0 then raise exception 'INVALID: % assertion(s) with a NULL pass value', v_null; end if;
  if v_n <> 39 then raise exception 'INCOMPLETE: % assertion rows, expected 39', v_n; end if;
end
$v$;
